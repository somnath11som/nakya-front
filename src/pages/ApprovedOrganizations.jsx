...
const ApprovedOrganizations = () => {
  ...
  const APPROVED_API_URL = "https://crp.mydevfactory.com/api/users/approved/organizations/";
  const API_URL_REJECT = "https://crp.mydevfactory.com/api/users/reject/";
  ...
  const fetchApprovedOrganizations = async () => {
    ...
    try {
      const response = await axios.get(APPROVED_API_URL, {
        headers: {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      if (selectedOrg?.id) {
        await axios.patch(API_URL_REJECT + selectedOrg?.id + "/", null, {
          headers: {
            Authorization: `Bearer ${tokens}`,
            "Content-Type": "application/json",
          },
  ...
  return (
    <div className="flex flex-col md:flex-row md:h-screen h-auto bg-black text-white">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 md:gap-0">
        ...
        <p className="text-gray-400 mb-6 text-base md:text-lg text-center md:text-left">Approved Organizations</p>
          ...
          <div className="border border-[#202020] rounded-lg overflow-x-auto max-h-[calc(80vh-100px)] overflow-auto scrollbar-hide">
            <table className="w-full text-white border border-[#202020] bg-[#171717]">
              <thead className="bg-[#000000] text-[#818181] sticky top-0">
                <tr>
                  <th className="py-3 px-5 border border-[#202020] text-left">ID</th>
                ...
                {approvedOrganizations
                  ...
                  .map((org) => (
                    <tr key={org.id} className="hover:bg-gray-800 transition duration-300">
                      <td className="py-3 px-5 border border-[#202020]">{org.id}</td>
                      ...
                      <td className="py-3 px-5 border border-[#202020]">Approved</td>
                      <td className="py-3 px-5 border border-[#202020] text-center">
                        <button onClick={() => handleStatusChange(org)} className="px-4 py-2 rounded bg-[#EA364F] hover:bg-red-600">
                          Reject
                        </button>
                      </td>
                    </tr>
        ...
        {isEditModalOpen && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-[#171717] p-6 rounded-lg w-96">
              ...
              <p className="text-white mb-4">Are you sure you want to reject {selectedOrg?.organization_name || `${selectedOrg?.first_name} ${selectedOrg?.last_name}`}?</p>
...
export default ApprovedOrganizations;
